    ...
    def run(self):
        ...
        try:
            ...
            if parsed.scheme not in ('http', 'https'):
                self.error.emit(self.generation, f"Invalid URL scheme: {parsed.scheme}")
                return
            ...
            # For local dev, allow unverified
...
class BLMClaimsManager(QObject):
    ...
    def _on_fetch_complete(self, generation: int, geojson_data: dict):
        ...
        for feat_data in features_data:
            ...
            geom = QgsGeometry.fromWkt(self._geojson_geom_to_wkt(geom_data))
            if geom.isNull():

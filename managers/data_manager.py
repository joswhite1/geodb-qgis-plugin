...
class DataManager:
    ...
    def _verify_record_in_project(self, model_name: str, record_id: int, project_name: str) -> bool:
        """Check that a record ID exists in the local layer for the given project."""
        layer = self.sync_manager._find_layer(model_name, project_name)
        if not layer:
            return False
        for feature in layer.getFeatures():
            if feature['id'] == record_id:
                return True
        return False
    ...
    def _validate_pull_response(self, features: list, schema) -> list:
        ...
        for feature in features:
            if not isinstance(feature, dict):
                self.logger.warning(f"Skipping non-dict feature: {type(feature)}")
                continue

            # Warn about unexpected fields (don't reject -- could be new server fields)
            unexpected = set(feature.keys()) - expected_fields - {'geometry', 'custom_fields'}
            if unexpected:
                self.logger.debug(f"Unexpected fields in response: {unexpected}")
    ...
    def pull_model_data(
        ...
        try:
            ...
            # Validate response features against schema
            from ..models.schemas import get_schema
            pull_schema = get_schema(model_name)
            features = self._validate_pull_response(features, pull_schema)

            # Validate deleted_ids: ensure it's a list of positive integers
            raw_deleted_ids = api_response.get('deleted_ids', [])
            deleted_ids = []
            if isinstance(raw_deleted_ids, list):
                for did in raw_deleted_ids:
                    if isinstance(did, int) and did > 0:
                        deleted_ids.append(did)
                    else:
                        self.logger.warning(
                            f"Ignoring invalid deleted_id: {did} (type={type(did).__name__})"
                        )
            else:
                self.logger.warning(
                    f"deleted_ids is not a list: {type(raw_deleted_ids).__name__}"
                )
            ...
            for i, feature in enumerate(features):
                try:
                    ...
                    if schema:
                        ...
                        # Validate field values before push
                        validation_errors = schema.validate_for_push(push_data)
                        if validation_errors:
                            self.logger.warning(
                                f"Validation errors for {model_name}: {validation_errors}"
                            )
                            errors.append({'feature': i, 'errors': validation_errors})
                            continue
    ...
    def upload_project_file(
        ...
        """
        Upload a file to the server as a ProjectFile.

        Args:
            file_path: Local path to the file (PNG, TIFF, etc.)
            name: Display name for the file
            category: Category code (IM, GL, GP, DM, etc.)
            description: Optional description
            is_raster: Whether this is a georeferenced raster
        ...
        Returns:
            Server response with created ProjectFile data
        """
        ...
        # Build form fields
        # Use project ID for multipart form data (NaturalKeyProjectField accepts int)
        fields = {
            'project': str(project.id),
            'name': name,
            'category': category,
            'is_raster': str(is_raster).lower(),
        }
        ...
        self.logger.info(
            f"Uploading ProjectFile: name={name}, category={category}, "
            f"is_raster={is_raster}, epsg={epsg}"
        )

        result = self.api_client.upload_file(url, file_path, fields)

        if progress_callback:
            progress_callback(100, "Upload complete!")

        self.logger.info(f"ProjectFile uploaded successfully: {result.get('id', 'unknown')}")
        return result
    ...
    def delete_record(
        ...
        # Verify record exists in local layer for active project (IDOR protection)
        if not self._verify_record_in_project(model_name, record_id, project.name):
            raise ValueError(
                f"Record {record_id} not found in active project '{project.name}'"
            )
    ...
    def queue_deletion(
        ...
        # Permission check
        if not self.project_manager.can_edit():
            self.logger.warning("Queue deletion rejected: no edit permission")
            return False

        # Verify record belongs to active project
        active_project = self.project_manager.get_active_project()
        if not active_project:
            self.logger.warning("Queue deletion rejected: no active project")
            return False

...
class ModelSchema:
    ...
    def validate_for_push(self, data: Dict[str, Any]) -> List[str]:
        """Validate field values before push. Returns list of error messages."""
        errors = []
        for f in self.get_writable_fields():
            ...
            # String length check
            if f.field_type == FieldType.STRING and f.length > 0:
                if isinstance(value, str) and len(value) > f.length:
                    errors.append(
                        f"Field '{f.name}' exceeds max length "
                        f"({len(value)} > {f.length})"
                    )

            # Coordinate bounds check
            if f.name == 'latitude' and isinstance(value, (int, float)):
                if not (-90 <= value <= 90):
                    errors.append(f"Latitude {value} out of range [-90, 90]")
            if f.name == 'longitude' and isinstance(value, (int, float)):
                if not (-180 <= value <= 180):
                    errors.append(f"Longitude {value} out of range [-180, 180]")
    ...
    try:
        ...
        for field_def in response.get('fields', []):
            ...
            custom_fields.append(FieldSchema(
                ...
                readonly=field_def.get('readonly', False),  # Respect server's flag

...
class ClaimsWizardState:
    ...
    def _extract_prefix_from_claims(self, path: str) -> Optional[str]:
        ...
        Claim names follow the pattern "PREFIX NUMBER" (e.g., "AMZ 1", "GE 2").
        This method finds the most common prefix across all claims.

        Args:
            path: Path to the GeoPackage file

        Returns:
            The extracted prefix, or None if no claims or no common prefix found
        """
        try:
            conn = sqlite3.connect(path)
            cursor = conn.cursor()

            # Check if lode_claims table exists
            cursor.execute('''
                SELECT name FROM sqlite_master
                WHERE type='table' AND name='lode_claims'
            ''')
            ...
            # Get all claim names
            cursor.execute('SELECT Name FROM lode_claims WHERE Name IS NOT NULL')
            ...
            # Extract prefixes - pattern is "PREFIX NUMBER" where prefix is letters
            # and number is digits, separated by space
            prefixes = []
            for name in names:
                # Match letters at the start, followed by space and digits
    ...
    def load_from_geopackage(self, path: str) -> bool:
        ...
        try:
            ...
            # Load prefix from metadata, but if it's the default "GE", try to
            # extract the actual prefix from existing claim names in the layer.
            # This handles legacy GeoPackages where prefix wasn't properly saved.
            metadata_prefix = metadata.get('grid_name_prefix', 'GE')
            if metadata_prefix == 'GE':
                extracted_prefix = self._extract_prefix_from_claims(path)
                if extracted_prefix:
                    self.grid_name_prefix = extracted_prefix
                else:
                    self.grid_name_prefix = metadata_prefix
            else:
                self.grid_name_prefix = metadata_prefix

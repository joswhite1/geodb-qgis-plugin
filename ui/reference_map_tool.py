    ...
    def _get_or_create_layer(self) -> Optional[QgsVectorLayer]:
        ...
        if self._layer:
            if is_layer_in_project(self._layer):
                ...
                # Layer was deleted or invalidated - clear the stale reference
                self.logger.info(
                    "[REFERENCE WIDGET] Clearing stale layer reference "
                    "(layer was removed from project or invalidated)"
                )
                self._layer = None

...
class ClaimsStep2Widget(ClaimsStepBase):
        ...
        try:
            ...
            if not project_crs.isValid() or not epsg_ok:
                QMessageBox.warning(
                    self,
                    "UTM Required",
                    "The project must be set to a UTM coordinate system before generating a claim grid.\n\n"
                    "Please go back to Step 1 (Setup) and use 'Auto-Detect UTM Zone' to set the correct CRS.\n\n"
                    "Generating claims in a non-UTM projection will produce incorrect claim boundaries."
                )
                return
    ...
    def _refresh_layers(self):
        ...
        logger.debug(f"[Step2] Refreshing layers, gpkg_path={gpkg_path}, state.claims_layer_id={self.state.claims_layer_id}")
        ...
        for layer_id, layer in QgsProject.instance().mapLayers().items():
            ...
            if gpkg_path:
                ...
                if source_path != gpkg_norm:
                    logger.debug(f"[Step2] Skipping layer '{layer.name()}' - not from GeoPackage (source: {source_path})")
                    continue

                logger.debug(f"[Step2] Adding layer to combo: '{layer.name()}' (id={layer_id})")
            self.layer_combo.addItem(layer.name(), layer_id)

        # Restore selection - prefer state.claims_layer_id (which may have been
        # updated by Step 5 to point to "Lode Claims"), fall back to current_id
        target_id = self.state.claims_layer_id or current_id
        if target_id:
            for i in range(self.layer_combo.count()):
                if self.layer_combo.itemData(i) == target_id:
                    self.layer_combo.setCurrentIndex(i)
                    logger.debug(f"[Step2] Selected layer by ID: {target_id}")
                    break
    ...
    def _get_selected_layer(self) -> Optional[QgsVectorLayer]:
        """Get the currently selected layer if it's still valid."""
    ...
    def save_state(self):
        ...
        if layer:
            self.state.claims_layer = layer
        elif not is_layer_valid(self.state.claims_layer):
            # Clear stale reference if stored layer is no longer valid
            self.state.claims_layer = None
    ...
    def load_state(self):
        ...
        # Clear stale layer reference if needed
        if not is_layer_valid(self.state.claims_layer):
            self.state.claims_layer = None

        # Try to select the saved layer from state
        # This ensures that if Step 5 updated claims_layer to "Lode Claims",
        # Step 2 will show that layer as selected when navigating back
        if self.state.claims_layer_id:
            ...
            # If the layer wasn't found by ID, it might be a newly created layer
            # that hasn't been added to the combo yet. Try to find by layer object.
            if not found and self.state.claims_layer:

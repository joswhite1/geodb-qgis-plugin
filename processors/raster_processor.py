...
class RasterProcessor:
    ...
    def load_xyz_tile_layer(
        ...
        Args:
            ...
            auth_token: Optional authentication token (deprecated, use auth_config_id)
            auth_config_id: Optional QGIS auth config ID for secure token-based auth
        ...
        try:
            ...
            # Add authentication using QGIS auth config (secure, no token in URL)
            if auth_config_id:
                uri_parts.append(f"authcfg={auth_config_id}")
            elif auth_token:
                # Fallback: embed token in URL (deprecated, for backward compat
                # until server supports header-based auth on tile endpoints)
                self.logger.warning(
                    "Using token in URL for tile auth (deprecated). "
                    "Update to auth_config_id when server supports header auth."
                )
            ...
            self.logger.debug(f"XYZ layer created for: {tile_url_template}")

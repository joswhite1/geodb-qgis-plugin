...
class ClaimsLayerGenerator:
    ...
    def _extract_claims_data(self, claims_layer: QgsVectorLayer) -> List[Dict[str, Any]]:
        ...
        # Sort features by Manual_FID (logical claim order) if available,
        # so that monument numbering (LM 1, LM 2...) matches claim order
        features = list(claims_layer.getFeatures())
        manual_fid_idx = claims_layer.fields().indexOf('Manual_FID')
        if manual_fid_idx >= 0:
            features.sort(key=lambda f: f.attribute(manual_fid_idx) or 0)
    ...
    def _snapshot_user_edits(
        self,
        project: QgsProject,
        new_layers: Dict[str, QgsVectorLayer]
    ) -> Dict[str, Dict[str, str]]:
        """
        Snapshot user-editable fields from existing Lode Claims layer before removal.

        Preserves user-added Notes so they survive layer recreation from server data.

        Returns:
            Dict mapping claim Name -> dict of preserved field values
        """
        if self.LODE_CLAIMS_LAYER not in new_layers:
            return {}

        preserve_fields = ['Notes']
        match_names = {self.LODE_CLAIMS_LAYER, self._get_display_name(self.LODE_CLAIMS_LAYER)}

        saved = {}
        for layer in project.mapLayers().values():
            if not isinstance(layer, QgsVectorLayer) or layer.name() not in match_names:
                continue
            for feature in layer.getFeatures():
                ...
                if edits:
                    saved[name] = edits
            break  # Only need the first matching layer

        if saved:
            self.logger.info(f"[CLAIMS] Saved user edits for {len(saved)} claim(s) before layer recreation")
        return saved

    def _restore_user_edits(
        self,
        layer: QgsVectorLayer,
        saved_edits: Dict[str, Dict[str, str]]
    ):
        """
        Restore user-editable fields onto a newly created Lode Claims layer.
        Only restores values where the new layer has an empty value (won't overwrite server data).
        """
        if not saved_edits or not layer or not layer.isValid():
            return
        ...
        for feature in layer.getFeatures():
            name = feature.attribute('Name') or feature.attribute('name') or ''
            if name not in saved_edits:
                continue
            for field_name, value in saved_edits[name].items():
                idx = layer.fields().indexOf(field_name)
                if idx >= 0:
                    current = feature.attribute(idx)
        ...
        if restored:
            self.logger.info(f"[CLAIMS] Restored user edits for {restored} field(s) on Lode Claims layer")
    ...
    def add_layers_to_project(
        ...
        try:
            ...
            # Snapshot user-editable fields (e.g. Notes) before destroying old layers
            saved_edits = self._snapshot_user_edits(project, layers)
            ...
            # Restore user-editable fields onto the new Lode Claims layer
            new_lode = layers.get(self.LODE_CLAIMS_LAYER)
            if new_lode and saved_edits:
                self._restore_user_edits(new_lode, saved_edits)
